...
As mentioned before, we're going to be using an ARM template to describe the resources needed and then deploy them.  The main resources we're going to be deploying are:
   ...
   - A single resource group that contains all the necessary resources including:
      - A single VMSS (VM Scale Set) containing 2 identical VM instances
      - A single Virtual Network with one subnet
      - A single NSG applied to the subnet that allows for remote access
